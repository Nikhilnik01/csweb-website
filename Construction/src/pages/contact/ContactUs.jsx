...
import { saveContactUs } from "../../services/api";
...
const ContactUs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const response = await saveContactUs(form);
      if (response.rs === 1 && response.res?.resultStatus === 1) {
        setSubmitted(true);
        window.scrollTo({ top: 0 });
      } else {
        throw new Error(response.rm || "Failed to send message");
      }
    } catch (err) {
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option>CITB Test Booking</option>
                        <option>NVQ Training</option>
                        <option>Construction Course</option>
                        <option>Bulk Test Booking</option>
